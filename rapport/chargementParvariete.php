...
// Construire la requête SQL avec filtres conditionnels
 $sql = "
                                        SELECT detailfilaosortie.*, facturesortie.id 
                                        FROM detailfilaosortie 
                                        INNER JOIN facturesortie ON detailfilaosortie.id_sortie = facturesortie.id 
                                        WHERE MONTH(facturesortie.date) = :mois AND detailfilaosortie.typ != 3
                                    ";

if ($selectedIdFou !== null) {
    $sql .= " AND facturesortie.id = :id";
}
...
if ($selectedIdFou !== null) {
    $selection->bindParam(':id', $selectedIdFou, PDO::PARAM_INT);
}
            ...
            <table class="table table-bordered table-striped table-hover">
                ...
                <tbody>
                    ...
                    foreach ($poissons as $poisson) {
                        ...
                        <tr>
                            <td><i class="fas fa-fish fa-lg text-info mr-3"></i> <strong><?= htmlspecialchars($poisson['id_poisson'])  ?></strong></td>
                            <td><?= htmlspecialchars($poisson['qtt']) ?></td>
                            <td><?= htmlspecialchars($poisson['sac']) ?></td>
                        </tr>
