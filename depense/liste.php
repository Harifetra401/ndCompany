...
$selection = $db->prepare("SELECT * FROM depence WHERE DATE_FORMAT(date, '%Y-%m') = :selected_month");
...
function poid_total($selected_month) {
    require('../db.php');
    $total = 0;
    $selection = $db->prepare("SELECT cout FROM depence WHERE DATE_FORMAT(date, '%Y-%m') = :selected_month");
    $selection->bindParam(':selected_month', $selected_month, PDO::PARAM_STR);
    $selection->execute();
    $fetchAll = $selection->fetchAll();
...
<div class="container mt-4">
    <div class="alert alert-info text-center">
        ...
        <strong><?= number_format(poid_total($selected_month), 2, ',', ' ') ?> AR</strong>
    </div>
</div>
<div class="container">
